...
const API_URL = 'https://stdfckbackend-zr1u.onrender.com/api/v1/admin/';
...
const verifyAccount = async (accountNumber, token) => {
  ...
  const response = await axios.get(`https://stdfckbackend-zr1u.onrender.com/api/v1/user/verify-account/${accountNumber}`, config);
  return response.data;
};
...
const updateFrozenStatus = async (userId, isFrozen, token) => {
  ...
  const response = await axios.put('https://stdfckbackend-zr1u.onrender.com/api/v1/admin/freeze' + `/${userId}`, { isFrozen }, config);
